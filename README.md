...
11. [ประโยชน์ที่ได้รับ](#11-ประโยชน์ที่ได้รับ)
...
|-------------|----------|--------|
| map | ทุกที่ | แปลงข้อมูลทุกตัวใน List โดยไม่แก้ List เดิม |
...
### จากการใช้ Functional Programming

- **โค้ดเทสต์ง่าย** : pure function ให้ผลลัพธ์เดิมเสมอเมื่อ input เดียวกัน
- **ไม่มี bug จาก shared state** : ข้อมูลเป็น immutable ไม่มีใครแก้ค่าที่คนอื่นใช้อยู่
- **Parallel ได้ง่าย** : แค่เติม `.par` เพราะไม่มี shared mutable state
- **อ่านเข้าใจง่าย** : เห็น data flow ชัดเจนว่า ข้อมูลเข้า แปลง ออก

### จากการทำ ETL Pipeline

- **ประมวลผลข้อมูลขนาดใหญ่ได้** : 135,000+ แถวอย่างมีระบบ
- **จัดการ error ได้ดี** : ไม่ crash เมื่อเจอข้อมูลเสีย
- **ตรวจสอบคุณภาพข้อมูลได้** : Postal Code vs City validation
- **เปรียบเทียบประสิทธิภาพได้** : รู้ว่าเมื่อไหร่ควรใช้ Parallel เมื่อไหร่ไม่ควร
